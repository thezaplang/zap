...
namespace zap
{
  ...
  public:
    ...
    constexpr T get_major() const noexcept{
      return maj;
    }

    constexpr T get_minor() const noexcept{
      return min;
    }

    constexpr T get_patch() const noexcept{
      return patch;
    }
    ...
    friend llvm::raw_ostream& operator<<(llvm::raw_ostream& os, const _version_base& v){
      return os<<v.get_major()<<'.'<<v.get_minor()<<'.'<<v.get_patch();
    }
  };
