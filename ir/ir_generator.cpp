...
void BoundIRGenerator::visit(sema::BoundFunctionDeclaration& node) {
    ...
    auto func = std::make_unique<Function>(symbol->name, symbol->returnType->toString());
    ...
    // Add parameters to the function and symbol map
    for (const auto& paramSymbol : symbol->parameters) {
        ...
        // Create an alloca for the parameter to make it mutable (standard LLVM practice)
        auto allocaReg = createRegister(std::make_shared<PointerType>(paramSymbol->type));
        currentBlock_->addInstruction(std::make_unique<AllocaInst>(allocaReg, paramSymbol->type));
...
void BoundIRGenerator::visit(sema::BoundVariableExpression& node) {
    ...
    if (it == symbolMap_.end()) {
        std::cerr << "Error: Symbol " << node.symbol->name << " not found in IR symbol map\n";
        return;
    }
    auto addr = it->second;
...
void BoundIRGenerator::visit(sema::BoundBinaryExpression& node) {
    ...
    currentBlock_->addInstruction(std::make_unique<BinaryInst>(op, reg, left, right));
    valueStack_.push(reg);
}
...
void BoundIRGenerator::visit(sema::BoundFunctionCall& node) {
    ...
    auto reg = createRegister(node.type);
    currentBlock_->addInstruction(std::make_unique<CallInst>(reg, node.symbol->name, args));
    valueStack_.push(reg);
}

std::shared_ptr<Value> BoundIRGenerator::createRegister(std::shared_ptr<Type> type) {
