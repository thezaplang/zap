...
run_runtime_test "tests/ptr_deref_complex_condition_test.zp" 0 "Pointer dereference in complex boolean conditions"
...
run_runtime_test "tests/string_view_owned_semantics_test.zp" 0 "StringView slice + owned copy semantics"
run_runtime_test "tests/string_compare_ops_test.zp" 0 "String equality operators (==, !=)"
run_runtime_test "tests/string_from_void_ptr_cast_test.zp" 0 "Cast *Void/*Char C-strings to String"
run_runtime_test "tests/string_textbuf_test.zp" 0 "TextBuf mutable string buffer helpers"
...
run_runtime_test "tests/enum_assigned_mixed_test.zp" 0 "Enum mixed explicit/implicit discriminants"
run_runtime_test "tests/enum_to_int_test.zp" 0 "Enum discriminant value via cast/toInt"
run_test "tests/enum_assigned_invalid_value_error.zp" 1 "Enum invalid assigned discriminant reports parse error"
...
run_runtime_test "tests/slice_param_test.zp" 0 "Array-view parameters ([]T)"
run_runtime_test "tests/slice_local_return_test.zp" 0 "Slice locals and returns ([]T)"
run_runtime_test "tests/slice_spread_test.zp" 0 "Slice spread into variadic (...slice)"
run_runtime_test "tests/slice_std_test.zp" 0 "std/slice helper operations"
...
run_test "tests/failable_try_outside_error.zp" 1 "Failable: ? is rejected outside failable return context"
run_test "tests/failable_try_error_type_mismatch.zp" 1 "Failable: ? requires matching propagated error type"
run_test "tests/failable_or_type_error.zp" 1 "Failable: or fallback must match success value type"
run_test "tests/failable_error_attr_required_error.zp" 1 "Failable: error type in T!E must be annotated with @error"
...
run_runtime_test "tests/ffi_stress_proj/main.zp" 0 "FFI stress project: multi-module qsort/callback/repr(C)/ext var"
run_runtime_test "tests/ffi_generic_stress/main.zp" 0 "FFI generic stress: multi-module + generics + qsort + repr(C)"
